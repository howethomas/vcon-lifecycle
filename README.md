...
[vCons][draft-vcon] are a powerful means to capture and collaborate on the details of human coversations, feeding AI systems, enabling entities to more accurately respond to customer needs, and to more effectively manage the health of their business. The vCon working group is about passing conversational data, such data commonly generated and collected in business and security environments, from chat logs to transcripts to recordings.  Most systems provide a way to store such information, but there are not many standards or interoperability within the storage or transmission mechanisms. [vcon] is a framework for capturing and collaborating on the details of a Virtual Conversation, feeding AI systems, enabling entities to more accurately respond to customer needs, and entities to more effectively manage the health of their business.   The two opposing forces influencing such information passing are trying to enforce privacy of personal data and providing the ability and interest to use conversations in various ways, e.g.,  AI analysis.  

Fundamental to the responsible protection of data and the ability to leverage that data is the proper management of consent. Consent granted by the data subject of a vCon also needs to be stored and passed, exactly like the other information contained in a vCon.  Unlike the rest of the information contained, the gathered consent is expressly not immutable.  Consent, when gathered for a purpose, can also be revoked by the data subject, at any time, and surely after a vCon has been shared for analysis. Multiple regulations, including the US born [California Consumer Protection Act (CCPA)][CCPA] and the EU born [General Data Protection Regulation (GDPR)][GDPR] outline requirements for entities to responsibly use and dispose of PII information upon request. Consent, although illustrative, is not the only example of mutability in the lifecycle of a vCon. Verification of parties, improvements in the analsys and additions of contextual attachments result in updates to a vCon after data is shared, begging for a mechanism to track and govern such changes. 

More so, the vCon is properly understood within the context of consent.  That is, the times and places vCons are created are vast, from consumer facing organizations, regulated industries and security applications. In each case, these conversations are collected under regulation and legal compliance. The requirement to prove and assure the presence of the proper governance of the conversation is generally assumed by those that use vCons; other uses are considered out of scope.  That is, the fundamental goal of vCons is to provide a definition of a conversation precisely so that third party tools and organizations can properly protect and support the digital rights of those recorded. In plain terms, vCons are a tool to enable good actors to "do the right thing" and to prove it, at scale.  Except for the implications their absence may imply, vCons do not detect actors unconcerned with the proper protection of the personal data.   

Although vCons are a tool to enable actors to do the right thing, it is not a tool that enables actors in a distributed system to prove it.  However, when combined with the SCITT protocol, proof becomes practical. SCITT provides Relying Parties to obtain information pertinent to the lifecycle of a vCon in a way that is "transparent".  SCITT achieves this by having producers publish information in a Transparency Service, where Relying Parties can check the information.  These relying parties can then use this information to make decisions based on the current state and context of the vCon, to account for changes in consent, updates in the accuracy of the content or ammendendment of the information it might contain. SCITT, the supply chain, integrity and trust protocol, enables clients to register statements about events, physical or virtual, such as when things are created or used.  These statements are immutable, and are useful to support auditing, governance and coordination between various distributed systems. 
